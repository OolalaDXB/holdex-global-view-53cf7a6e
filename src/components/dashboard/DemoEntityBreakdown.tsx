...
interface DemoEntityBreakdownProps {
  displayCurrency: string;
  delay?: number;
  isBlurred?: boolean;
}

export function DemoEntityBreakdown({ displayCurrency, delay = 0, isBlurred = false }: DemoEntityBreakdownProps) {
  ...
  assets.forEach(asset => {
    ...
    const allocation = (asset as any).ownership_allocation as any[] | null;
  ...
  collections.forEach(collection => {
    ...
    const allocation = (collection as any).ownership_allocation as any[] | null;
  ...
  const handleEntityClick = (entityId: string) => {
    if (entityId === 'unassigned') {
      navigate('/demo/assets?entity=unassigned');
    } else {
      navigate(`/demo/assets?entity=${entityId}`);
    }
  };
            ...
            >
              <div className="flex justify-between items-center text-sm">
                <div className="flex items-center gap-2">
                  {entity ? (
                    <>
                      <EntityIcon 
                        iconName={(entity as any).icon || getEntityIconName(entity.type)} 
              ...
              <div className="stat-bar">
                <div 
                  className={cn(
                    "stat-bar-fill",
                    (entity as any)?.color ? '' : 'bg-muted-foreground'
                  )}
                  style={{ 
                    width: `${percentage}%`,
                    backgroundColor: (entity as any)?.color || undefined,
